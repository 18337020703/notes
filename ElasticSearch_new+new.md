...
```shell
[root@VM_0_9_centos ~]# docker pull elasticsearch:6.4.3
```

+ 运行elasticsearch

```shell
[root@VM_0_9_centos ~]# docker run -d --name es -p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" elasticsearch:6.4.3
或者
[root@VM_0_9_centos ~]# docker run -e ES_JAVA_OPTS="-Xms256m -Xmx256m" -d -p 9200:9200 -p 9300:9300 --name es elasticsearch:6.8.7
```

+ 修改配置文件

```shell
[root@VM_0_9_centos ~]# docker exec -it es /bin/bash
[root@4d33d47021b7 elasticsearch]# cd config
[root@4d33d47021b7 config]# vi elasticsearch.yml
 //添加如下配置
http.cors.enabled: true   //临时能以root用户启动，es默认是普通用户开启
http.cors.allow-origin: "*"
//访问 http://49.234.18.34:9200 测试
```
...
### 8.1 docker 安装 kibana

```shell
1.[root@VM_0_9_centos ~]# docker pull kibana:6.8.7         //版本要与es相同
2.[root@VM_0_9_centos ~]# docker run -it -d -e ELASTICSEARCH_URL=http://49.234.18.34:9200 --name kibana -p 5601:5601 kibana:6.8.7
3.[root@VM_0_9_centos ~]# docker ps
CONTAINER ID        IMAGE                 COMMAND                  CREATED             STATUS              PORTS                                            NAMES
d6c79f964ddf        kibana:6.8.7          "/usr/local/bin/ki..."   10 seconds ago      Up 8 seconds        0.0.0.0:5601->5601/tcp                           kibana
4.访问 http://49.234.18.34:5601
```
...
```shell
1.进入容器目录
  [root@VM_0_9_centos ~]# docker exec -it 容器Id /bin/bash
2.[root@b4cd480a05f9 elasticsearch]# ls
  LICENSE.txt  NOTICE.txt  README.textile  bin  config  data  lib  logs  modules  plugins
3.[root@b4cd480a05f9 elasticsearch]# cd plugins/
4.[root@b4cd480a05f9 plugins]# mkdir analysis-ik
5.[root@b4cd480a05f9 plugins]# cd analysis-ik/
5.[root@b4cd480a05f9 analysis-ik]# wget https://github.com/medcl/elasticsearch-analysis-   ik/releases/download/v6.8.7/elasticsearch-analysis-ik-6.8.7.zip   //版本要与es一致
6.[root@b4cd480a05f9 plugins]# unzip elasticsearch-analysis-ik-6.8.7.zip 
7.重启es
```
...
> [https://mvnrepository.com](https://mvnrepository.com/)查找Jest与Es相对应的版本
**嗯，Jest Client 过时了，不推荐使用了，fuck**
